...
library(readxl)
Gamesonyourphone_ <- read_excel("/cloud/project/extra/Gamesonyourphone?.xlsx")
```
...
```{r violin-plot, echo = F, warning = F, message = F, fig.alt = "This violin plot shows a violin shape corresponding to each group of playing times. The average of each group is represented as the widest part of the violin, while the tails represent the spread of the data. It seems that longer playing games tend to have higher ratings and shorter spreads, so most people enjoyed these games, while shorter playtimes had more variety.", out.width = "70%", out.height= "70%", fig.align='center'}
board_games %>% 
...
### By Year Published
```{r best-boardgame, echo = F, warning = F, message = F, fig.alt = "This scatterplot shows a dot representing each game in the dataset under 100 minutes long. It then plots these points based on average rating and users rated, and includes a color scale that shows hwo long each game was from 0-100 miniutes. We found that most points are generally higher rated and the more users rating the game, generally means it will have a higher average rating.", out.width = "80%", out.height= "70%", fig.align = 'center'}
...
``` {r cat-pie-chart, echo=FALSE, warning=FALSE, out.width='80%', alt.text = "Pie chart showing the distrobution of game mechanics preferred by DCS 210, broken down by proportion. The most preferred game mechanic was Area Control makig up 33.3 percent of responses. Dice rolling and card drafting were the next more popular, with 20 percent of the class preferred those mechanics."} 
...
ggplot(data = prop_pie_cat, mapping = aes(x = "", y = cat_proportion, fill = categories)) + 
    ...
    geom_text(aes(label = paste0(cat_proportion, "%", categories)),
...
ggplot(data = prop_pie, mapping = aes(x = "", y = proportion, fill = mechanics)) + 
    geom_bar(stat = "identity", color = "black") + 
    coord_polar(theta = "y", start = 0) + 
    geom_text(aes(label = paste0(proportion, "%", mechanics)),
              position = position_stack(vjust = 0.5)) + 
    scale_fill_manual(values = c("#D73027", "#FC8D59",
